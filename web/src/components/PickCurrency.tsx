 ...
 * because AV exists for every position in every year and career EPA does not —
 * a lineman never touches the ball and so never scores any. That choice is
 * already defended here, but it has a consequence nobody states: AV and EPA
 * price the top of the draft completely differently, and the whole of the gap
 * is quarterbacks.
 */
export function PickCurrency({
  ...
  const restEpa = restPicks
    ? rest.reduce((a, b) => a + b.epa * b.n, 0) / restPicks
    : 0;
  const peak = Math.max(...mix.map((m) => Math.max(m.epa, 0.01)), 0.01);
    ...
    >
      <div className="px-4 pt-3">
        <ChartExport
          ...
          caption="Expected career return by pick · AV vs EPA"
        >
        <LineChart
          ...
          series={[
            ...
            },
            {
              name: "EPA",
              color: "var(--c1)",
              values: shown.map((c) => c.epa_relative * 100),
            },
          ]}
        />
        </ChartExport>
      ...
      <div className="px-4 py-3 grid gap-3 grid-cols-[repeat(auto-fit,minmax(150px,1fr))] border-t border-rule">
        ...
        <div>
          ...
          <div className="text-[11px] text-ink-3">in EPA</div>
        </div>
        {qb && (
          <div>
            <div className="label">Top-10 quarterback</div>
            <div className="num text-[22px] font-semibold mt-0.5">{num(qb.epa, 0)}</div>
            <div className="text-[11px] text-ink-3">career EPA · {qb.n} picks</div>
          </div>
        )}
        {rest.length > 0 && (
          <div>
            <div className="label">Top-10, everyone else</div>
            <div className="num text-[22px] font-semibold mt-0.5">{num(restEpa, 0)}</div>
            <div className="text-[11px] text-ink-3">career EPA · {restPicks} picks</div>
          </div>
        )}
      </div>
          ...
          <table className="grid-table">
            <thead>
              <tr>
                ...
                <th>Career EPA</th>
                <th>Career AV</th>
                <th className="l" style={{ width: 150 }} aria-label="Career EPA returned" />
              </tr>
            </thead>
            <tbody>
              {mix.map((m) => (
                <tr key={m.position}>
                  ...
                  <td className="num font-semibold">{num(m.epa, 0)}</td>
                  ...
                  <td className="l">
                    <span className="relative block h-[11px] bg-panel-2 rounded-[2px]">
                      <span
                        ...
                        style={{
                          width: `${(Math.max(0, m.epa) / peak) * 100}%`,
                          background:
      ...
      <div className="px-4 py-2.5 border-t border-rule text-[11.5px] text-ink-2">
        ...
        top-ten interior lineman as near-equals; EPA has the quarterback ahead by an order of
        magnitude, because AV deliberately compresses the gap between positions and EPA does
        not. A top pick&apos;s value in expected points is therefore mostly an{" "}
        <em>option on a quarterback</em> — which is worth knowing before reading the AV curve
        above as though every slot were fungible. EPA scores only the man who touched the ball,
        so a lineman returns zero here by construction, not by failure.
      </div>
    </Panel>
  );
}
